      ...
      <div className="max-w-7xl mx-auto flex h-14 items-center px-4 sm:px-6 lg:px-8">
        <div className="mr-4 hidden md:flex">
          ...
          <nav className="flex items-center space-x-6 text-sm">
            {header.nav?.items?.map((v: Item, idx: number) => {
              return (
                <Link key={v.url || idx} href={v.url || ""} target={v.target || "_self"}>
                  {v.title}
                </Link>
              );
            })}
        ...
        <div className="flex flex-1 items-center justify-between space-x-4 md:justify-end">
          {header.buttons?.map((button, idx) => (
            <Link 
              key={`button-${button.url || idx}`} 
              href={button.url || ""} 
              target={button.target || "_self"}
            >
              <Button
                size="sm"
                variant={button.theme === "outline" ? "outline" : "default"}
              >
                {button.title}
              </Button>
            </Link>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  {header.nav?.items?.map((item: Item, idx: number) => (
                    <DropdownMenuItem key={`dropdown-${item.url || idx}`}>
                      <Link
                        href={item.url || ""}
